...
# 1) Cargar el índice de agua con un umbral cualquiera (solo importa el índice)
archivo = "./recortes_acolite/2025-06-09.tif"
indice, _, _, _ = procesar_tif(archivo, umbral=0.0)

# 2) Calcular histograma del índice
hist, bins = np.histogram(indice.flatten(), bins=100, range=(-1, 1))

# 3) Buscar mínimos locales para sugerir umbral
minimos = [
...
# 4) Graficar histograma y marcar el umbral
plt.plot(bins[:-1], hist)
plt.axvline(umbral_sugerido, color="red", lw=2,
            label=f"Umbral ≃ {umbral_sugerido:.3f}")
plt.xlabel("Valor del índice de agua")
plt.ylabel("Frecuencia")
plt.title("Histograma del Índice de Agua")
plt.legend()
plt.tight_layout()
plt.show()
...
display(Math(
    ...
    r"\left(\frac{B05}{B04}\right)^{-0.371} "
    r"\cdot B02^{-0.091}"
))
