...
### Selección de variables mediante Forward Selection con AIC usando validación cruzada
...
from modelo_multivariable_completo import ModeloMultivariableCompleto
import pandas as pd

# Cargar datos
datos = pd.read_csv("datos/datos_sol_sus_acolite.csv")

# Inicializar el modelo con los datos
modelo = ModeloMultivariableCompleto(datos)

# 1) Selección hacia adelante con validación cruzada
modelo.forward_selection()

# 2) Ajuste del modelo final con la mejor combinación
modelo.modelo_final = modelo.ajustar_modelo_final()  # guardar el modelo ajustado

# 3) Mostrar métricas, ecuaciones y gráficos
...
### Selección de variables mediante Forward Selection con AIC usando bootstrap
...
import sys
sys.path.append("scripts")
from modelo_multivariable_boot import ModeloMultivariableCompleto
import pandas as pd

# 1) Cargar datos
datos = pd.read_csv("datos/datos_sol_sus_acolite.csv")

# 2) Inicializar el modelo con los datos (aquí n_boot=200)
modelo = ModeloMultivariableCompleto(datos, max_features=3, n_boot=200)

# 3) Selección hacia adelante usando bootstrap y AIC
modelo.forward_selection()

# 4) Ajuste del modelo final con la mejor combinación seleccionada
modelo.modelo_final = modelo.ajustar_modelo_final()

# 5) Mostrar tablas, ecuaciones, métricas, intervalos de confianza y gráficos
modelo.mostrar_resultados()
```
...
# Agregar carpeta "scripts" al path de Python
sys.path.append("scripts")

# Importar la función desde rgb_utils.py
from rgb_utils import make_rgb

# Ruta al archivo .tif
ruta_tif = "./recortes_acolite/2025-06-09.tif"

# Generar la imagen RGB y la info
rgb, info = make_rgb(ruta_tif)

# Mostrar info en consola / notebook
print("Bandas elegidas:", info['sel'])

# Mostrar imagen en el reporte
plt.imshow(rgb)
plt.axis("off")
plt.show()
```
...
# Agregar la carpeta scripts al path para poder importar el módulo
sys.path.append("scripts")

# Importar la función desde modelo_tif_gkf.py
from modelo_tif_gkf import procesar_tif  

# Mostrar la ecuación en LaTeX
display(Math(r"\text{sol\_sus} = 61.7089 \cdot \left(\frac{B02}{B07}\right)^{-0.8370} \cdot \left(B8A^3\right)^{-0.0044}"))

archivo = "./recortes_acolite/2025-06-09.tif"
umbrales = [0]  # Se puede agregar más valores 

for u in umbrales:
    indice, mascara, parametro, perfil = procesar_tif(archivo, umbral=u)

    print(f"\n--- Umbral: {u} ---")
    print("Estadísticas del índice de agua:")
    print(f"  Min: {np.nanmin(indice):.4f}, Max: {np.nanmax(indice):.4f}, Media: {np.nanmean(indice):.4f}")

    print("Estadísticas de la máscara de agua:")
    print(f"  Valores únicos: {np.unique(mascara)}  (1=agua, 0=tierra, 255=nodata)")

    print("Estadísticas del parámetro sólidos suspendidos:")
    # Excluir valores nodata = -9999.0
    validos = parametro != -9999.0
    print(f"  Min: {np.nanmin(parametro[validos]):.4f}, Max: {np.nanmax(parametro[validos]):.4f}, Media: {np.nanmean(parametro[validos]):.4f}")

    fig, axs = plt.subplots(1, 3, figsize=(15, 5))
    fig.suptitle(f"Resultados para umbral = {u}", fontsize=16)

    # Índice de Agua
    im0 = axs[0].imshow(indice, cmap='coolwarm', vmin=-1, vmax=1)
    axs[0].set_title("Índice de Agua")
    fig.colorbar(im0, ax=axs[0], fraction=0.046, pad=0.04)
    axs[0].axis('off')

    # Máscara de Agua: visualizar como binaria 0–1
    mask_vis = np.where(mascara == 1, 1, 0)  
    im1 = axs[1].imshow(mask_vis, cmap='gray', vmin=0, vmax=1)
    axs[1].set_title("Máscara de Agua")
    fig.colorbar(im1, ax=axs[1], fraction=0.046, pad=0.04)
    axs[1].axis('off')

    # Sólidos Suspendidos
    vmax_sol = np.nanpercentile(parametro[validos], 98)  # Excluye nodata
    im2 = axs[2].imshow(parametro, cmap='inferno', vmin=0, vmax=vmax_sol)
    axs[2].set_title("Sólidos Suspendidos Estimados")
...
# 1) Cargar índice
archivo = "./recortes_acolite/2025-06-09.tif"
indice, _, _, _ = procesar_tif(archivo, umbral=0.0)

# 2) Histograma
hist, bins = np.histogram(indice.flatten(), bins=100, range=(-1, 1))

# 3) Mínimos locales para sugerir un umbral
minimos = [
    i for i in range(1, len(hist)-1)
    if hist[i] < hist[i-1] and hist[i] < hist[i+1]
]
if minimos:
    idx_min = minimos[len(minimos)//2]
    umbral_sugerido = (bins[idx_min] + bins[idx_min+1]) / 2
else:
    umbral_sugerido = 0.0
...
# 4) Dibujo
fig, ax = plt.subplots(figsize=(4.5, 2.0), dpi=150)
ax.plot(bins[:-1], hist, lw=1.2)

# Guardamos la referencia de la línea del umbral
linea_umbral = ax.axvline(umbral_sugerido, color="red", lw=1.4)

# Leyenda vinculada a la línea real (color y estilo correctos)
ax.legend([linea_umbral],
          [f"Umbral ≃ {umbral_sugerido:.3f}"],
          loc="upper left",
          bbox_to_anchor=(-0.03, 1.02),
          fontsize=8,
          frameon=False)

ax.set_xlabel("Índice", fontsize=9)
ax.set_ylabel("Frecuencia", fontsize=9)
ax.set_title("Histograma del Índice de Agua", fontsize=10)
...
archivo = "./recortes_acolite/2025-06-09.tif"
umbrales = [0.25, 0.33, 0.35, 0.40]  # se puede agregar más

for u in umbrales:
    # Procesar con el umbral actual
    indice, mascara, parametro, perfil = procesar_tif(archivo, umbral=u)
    ...
    # 2) Máscara de Agua  
    # Convertimos 255 → 0 para visualizar sólo binario
    mask_vis = np.where(mascara == 1, 1, 0)
    im1 = axs[1].imshow(mask_vis, cmap='gray', vmin=0, vmax=1)
    axs[1].set_title("Máscara de Agua")
    fig.colorbar(im1, ax=axs[1], fraction=0.046, pad=0.04)
    ...
    # 3) Sólidos Suspendidos Estimados  
    # Excluir nodata = -9999.0 antes de calcular el 98º percentil
    valid = parametro != -9999.0
    vmax_sol = np.nanpercentile(parametro[valid], 98)
    im2 = axs[2].imshow(parametro, cmap='inferno', vmin=0, vmax=vmax_sol)
    axs[2].set_title("Sólidos Suspendidos Estimados")
    fig.colorbar(im2, ax=axs[2], fraction=0.046, pad=0.04)
    axs[2].axis('off')
...
import sys
sys.path.append("scripts")
from modelo_tif_vc import procesar_tif  
import matplotlib.pyplot as plt
import numpy as np
from IPython.display import display, Math

# Mostrar la ecuación en LaTeX 
display(Math(r"\mathrm{sol\_sus} = 86.402 \cdot \left(\tfrac{B04}{B05}\right)^{-0.424} \cdot B04^{-0.116}"))
...
archivo = "./recortes_acolite/2025-06-09.tif"
umbrales = [0.25, 0.33, 0.35, 0.40]  # se puede agregar
...
import sys
sys.path.append("scripts")
from modelo_tif_bt import procesar_tif  
import matplotlib.pyplot as plt
import numpy as np
from IPython.display import display, Math

# Mostrar la ecuación en LaTeX con la nueva fórmula:
display(Math(
    r"\mathrm{sol\_sus} = 86.402 \cdot "
    r"\left(\frac{B04}{B05}\right)^{-0.371} "
    r"\cdot B02^{-0.091}"
))

archivo = "./recortes_acolite/2025-06-09.tif"
umbrales = [0]  # Umbral de ejemplo
...
for u in umbrales:
    ...
    print(cat)
    print("Estadísticas del índice de agua:")
    print(f"  Min: {np.nanmin(indice):.4f}, Max: {np.nanmax(indice):.4f}, "
          f"Media: {np.nanmean(indice):.4f}")

    print("Estadísticas de la máscara de agua:")
    print(f"  Valores únicos: {np.unique(mascara)}  (1=agua, 0=tierra, 255=nodata)")

    # Estadísticas de sol_sus (excluyendo nodata = -9999.0)
    validos = parametro != -9999.0
    print("Estadísticas del parámetro sólidos suspendidos:")
    print(f"  Min: {np.nanmin(parametro[validos]):.4f}, "
          f"Max: {np.nanmax(parametro[validos]):.4f}, "
          f"Media: {np.nanmean(parametro[validos]):.4f}")
    ...
    # Sólidos suspendidos
    vmax_sol = np.nanpercentile(parametro[validos], 98)
    im2 = axs[2].imshow(parametro, cmap="inferno", vmin=0, vmax=vmax_sol)
    axs[2].set_title("Sólidos Suspendidos Estimados")
    fig.colorbar(im2, ax=axs[2], fraction=0.046, pad=0.04)
    axs[2].axis("off")
