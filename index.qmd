...
- **R² bajo (< 0.20)** en todas las bandas, lo cual indica que **ninguna banda aislada** explica buena parte de la variabilidad de los sólidos suspendidos.
- **R² ajustado** muy similar a R² en las bandas con mejor desempeño (B08, B07, B05, B06), y **negativo** en las bandas con R² casi nulo (B03, B02, B01, B12, B11).  
- **Un R² ajustado negativo** implica que usar la media de la variable (sin modelo) sería más eficaz que la regresión lineal simple.

**Error de predicción (RMSE):**

- **RMSE ≈ 25** para las cuatro mejores bandas (B08–B06), es decir, el error medio de predicción es de unas 25 unidades de `sol_sus`.  
- Las bandas con peor R² (B11, B12) presentan **RMSE ≈ 27.5**, casi equivalente a predecir siempre la media.
...
- Las bandas B05, B07, B08 y B06 muestran ahora un R² (log) entre 0.25 y 0.26, un aumento relativo frente al R² original (~0.16–0.17).
...
- La relación entre reflectancia y sólidos suspendidos se ajusta un poco mejor tras la transformación log–log, sugiriendo una dependencia multiplicativa o de potencia más que estrictamente lineal.
...
Este ranking coincide con el análisis sin log.

**R² ajustado:**

La diferencia entre R²_log y R²_ajustado_log es pequeña (< 0.02) para las bandas con mejor desempeño, lo que indica que el ajuste mejora genuinamente y no es un artefacto por el número de variables.
...
Nunca una sola banda explicó mucho (> 0.26 de R²_log), pero quizá muchas juntas sí captan gran parte de la variabilidad.


#### Incorporar AIC en la selección de modelo

El AIC penaliza por complejidad y te ayudará a decidir si conviene agregar o quitar bandas.
